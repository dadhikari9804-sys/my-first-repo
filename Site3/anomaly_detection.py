...
class automated_prediction:
    ...
    def LSTM_prediction(self):
        ...
        self.lstm_model = load_model(self.lstm_filepath, custom_objects={'mse': 'mse', 'mae': 'mae'})
        ...
        self.lstm_input, self.df_lstm = LSTM_3D_Input(scaled_data, df_scaled, self.time_steps)
        ...
        lstm_pred = self.lstm_model.predict(self.lstm_input)
        mae_loss = np.mean(np.abs(flatten_to_2d(lstm_pred) - flatten_to_2d(self.lstm_input)), axis = 1)
        ...
        df_machine = reconstructed_df(mae_loss, self.df_lstm, threshold)
